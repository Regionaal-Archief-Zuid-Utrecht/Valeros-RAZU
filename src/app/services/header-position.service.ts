...
import { HeaderPosition, HeaderSettings, HeaderVerticalPosition } from '../models/header/header-position.types';
...
export class HeaderPositionService {
  ...
  getPosition(settings: HeaderSettings): HeaderPosition {
    ...
    const isStickingSide = (isStickingLeft || isStickingRight) && !this.isNarrowScreen();
  ...
  private getEffectiveVerticalPosition(settings: HeaderSettings): HeaderVerticalPosition {
    ...
    const isStickPosition = ['stick-to-left', 'stick-to-right'].includes(horizontalPosition);
    if (!isStickPosition) {
      return verticalPosition === 'bottom' ? 'bottom' : 'top';
    }
  ...
  getPositionClasses(settings: HeaderSettings): string {
    ...
    const smallScreenVertical = verticalPosition === 'bottom' ? 'bottom-0' : 'top-0';
    classes.push(smallScreenVertical);

    // For wider screens, apply the configured vertical position
    const useMiddle = verticalPosition === 'middle' && 
  ...
  getRoundedClasses(settings: HeaderSettings): string {
    ...
    if (!position.isStickingSide) {
      ...
      return position.effectiveVerticalPosition === 'top' ? 'rounded-b-lg' : 
             position.effectiveVerticalPosition === 'bottom' ? 'rounded-t-lg' : 
             'rounded-lg';
    }
  ...
  getPlaceholderClasses(settings: HeaderSettings, headerHeight: string, contentGap: string): string {
